    ...
    <div className="space-y-3">
      ...
      <div className="h-[23rem] md:h-[23.5rem]">
        ...
        >
          ...
          />
          <div className="space-y-3">
            <h1 className="font-medium leading-6 text-lg text-black dark:text-white dark:group-hover:!text-gray-300 duration-300 group-hover:!text-gray-600">
              {result.title.length > 80 ? result.title.substring(0, 80) + "..." : result.title}
            </h1>
            <h3 className="dark:text-gray-400 text-gray-500 !leading-5">
              {convertToPlain(result.content).length > 135
                ? convertToPlain(result.content).substring(0, 135) + "..."
      ...
      <div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-1 items-center">
            {/* dots */}
            <More title={result.title} id={result._id} result={result}/>
          </div>
        </div>
      </div>
    </div>
